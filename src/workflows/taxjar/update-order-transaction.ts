...
export const updateOrderTransactionWorkflow = createWorkflow(
  "update-order-transaction-workflow",
  (input: WorkflowInput) => {
    ...
    const transactionInput = transform({ orders }, ({ orders }) => {
      ...
      const input: UpdateOrderParams = {
        transaction_id: "Taxjar-" + (orders[0]?.id ?? ""),
        transaction_date:
          (orders[0]?.updated_at as string) ?? new Date().toISOString(),
        to_country:
