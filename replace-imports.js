...
const replacements = [
  ...
  {
    pattern:
      /from\s+['"]@?opentelemetry\/(instrumentation-pg|resources|sdk-node|sdk-trace-node)['"]/g,
    // eslint-disable-next-line quotes
  ...
  {
    pattern:
      /require\s*\(\s*['"]@?opentelemetry\/(instrumentation-pg|resources|sdk-node|sdk-trace-node)['"]\s*\)/g,
    // eslint-disable-next-line quotes
...
function getTargetFiles() {
  try {
    ...
    console.log(
      `📄 Scanning ${files.length} files for target imports and require statements...`,
    );
...
function main() {
  ...
  if (targetFiles.length === 0) {
    console.log(
      "ℹ️  No files found with target imports or require statements.",
    );
    return;
  }
